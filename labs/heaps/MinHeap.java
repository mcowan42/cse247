...
public class MinHeap<T extends Comparable<T>> implements PriorityQueue<T> {
	 ...
	 *   inserting into the heap is reducible to calling percolateUp on the
	 ...
	 *   percolateUp(int loc) when necessary.
	 */
	public Decreaser<T> insert(T thing) {
		...
		//   Recall in class we inserted and percolated up.
		//
		// FIXME
		//
		array[size]=ans;
	...
	void percolateUp(int loc) {
		...
		int j;
		j=i/2;
		while ((i != 1) && (array[i].getValue().compareTo(array[j].getValue())<0)){
			array[0]=array[i];
			array[i]=array[j];
			array[j]=array[0];
			array[i].loc=i;
	 ...
	 */
	private void percolateDown(int where) {
		//
		// As described in lecture
		//  FIXME
		//
		...
		while((i < size) && (2*i <= size)){
			if (2*i+1 <= size){
				if ((array[i].getValue().compareTo(array[2*i].getValue())>0) || 
						(array[i].getValue().compareTo(array[2*i+1].getValue())>0)){
					if (array[2*i].getValue().compareTo(array[2*i+1].getValue())<=0){
						array[0]=array[i];
						array[i]=array[2*i];
						array[2*i]=array[0];
						array[i].loc=i;
